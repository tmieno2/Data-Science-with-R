...
+ Using `future_lapply()` read all the files (this is not a good use case of parallelized processing)
...
+ Write a function that 
  * reads "corn_experiment_1.rds"
  * run OLS of `corn_yield` on `log(N_rate)`
  * 
...
Grid search basically evaluates the objective function (profit here) at many levels of decision variables (nitrogen here) and pick the one that maximizes the objective function (profit).
...
```{r gg-data, echo = F, out.width = "60%", fig.dim = c(4,2)}
ggplot(data_main) +
...
You can use `filter()` combined with `max()` to identify the optimal N rate.
