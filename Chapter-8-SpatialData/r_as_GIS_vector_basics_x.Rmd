...
Read the "Fairway_Five" shape files onto R using `st_read()`, and bame it `Fairway_Five`.
...
wells <- readRDS('./Data/well_registration.rds')  %>% 
	select(wellid, ownerid, acres, longdd, latdd)
)
``` 
...
The CRS information is missing!
...
+ So, right after the conversion to `sf`, the `sf` object is NA. 

+ Obviously, $R$ does not know the reference system without you telling it. 

.content-box-green[**Solution**]

+ We know (yes, YOU need to know the CRS of your data) that the geographic coordinates in the wells data is NAD 83 $(epsg=4269)$ for this dataset. 

+ We can assign the right CRS using either `st_set_crs()` or `st_crs()`.
...
.content-box-green[**See the changes**]
...
Read "fwy05_grid_NAD" shape files, name it `fwy05_grid`, transform its CRS to NAS 83/UTM zone 17N (its EPSG code is 26917)
