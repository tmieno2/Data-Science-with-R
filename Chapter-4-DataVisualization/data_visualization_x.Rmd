...
+ figures where its main body are made color differentiated (e.g., with `aes(color = var)` inside the `geom_*()` function)
+  faceted figures  
...
```{r diverging2, echo = F, out.width = "60%"}  
county_yield %>% 
...
`r chunk_reveal(
  ...
  title = "# Diverging chart: little by little",
  break_type = "user", 
  display_type = "both", 
  left_assign = FALSE,
  width = c(40, 60)
...
```{r area-chart, echo = F, out.width = "60%"}  
county_yield %>% 
  filter(!is.na(corn_yield)) %>% 
  group_by(state_name, year) %>% 
  summarize(corn_yield = mean(corn_yield)) %>% 
...
county_yield %>% #BREAK
  ...
  summarize(corn_yield = mean(corn_yield)) %>% #BREAK 
  group_by(year) %>% 
  mutate(corn_yield_p = corn_yield / sum(corn_yield)) %>% #BREAK
  ggplot(data = .) + #BREAK
  geom_area(
    aes(
      y = corn_yield_p, 
  ...
  labs(x = "Year", y = "Corn Yield Ratio") + #BREAK
...
`r chunk_reveal(
  ...
  title = "# Area chart: little by little",
  break_type = "user", 
  display_type = "both", 
  left_assign = FALSE,
  width = c(50, 50)
...
`r chunk_reveal(
  ...
  title = "# Mean and SE chart: little by little",
  break_type = "user", 
  display_type = "both", 
  left_assign = FALSE,
  width = c(50, 50)
...
data <- fread("N_Response_total_ordered-1.csv") %>% 
  select(x, matches("_BL_"), matches("_ML_"), matches("_TL_")) %>% 
  pivot_longer(-x, names_to = "gene", values_to = "value") %>% 
  mutate(group = case_when(
    str_detect(gene, "_BL_") ~ "BL",
    str_detect(gene, "_ML_") ~ "ML",
    str_detect(gene, "_TL_") ~ "TL"
  )) %>% 
  mutate(gene_txt = gsub("_BL_", "", gene)) %>% 
  mutate(gene_txt = gsub("_ML_", "", gene_txt)) %>% 
  mutate(gene_txt = gsub("_TL_", "", gene_txt)) %>% 
  #--- I am drawing random numbers here  ---#
  # But, this is where you normalize the value with log and z
